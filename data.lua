local function png(name)
    return ("__ai_combinator__/graphics/%s.png"):format(name)
end
...
for k, spec in pairs(ai_combinator.sprites) do
    for n, layer in pairs(spec.layers) do
        ...
        if not layer.filename:match("%-shadow%.png$") then
            layer.filename = png("ai-combinator-sprites")
        else
            layer.filename = png("ai-combinator-sprites-shadow")
        end
    end
end
...
for prop, sprites in pairs(ai_combinator) do
    if not prop:match("_symbol_sprites$") then
        goto skip
    end
    for dir, spec in pairs(sprites) do
        sprites[dir] = spec
        if spec.filename ~= "__base__/graphics/entity/combinator/combinator-displays.png" then
            error("decider-combinator display symbols sprite sheet incompatibility detected")
        end
...
data:extend({
    ...
    -- Key bindings
    { type = "custom-input", name = "ai-combinator-code-save", key_sequence = "CONTROL + S", order = "03" },
    { type = "custom-input", name = "ai-combinator-code-commit", key_sequence = "CONTROL + RETURN", order = "04" },
    { type = "custom-input", name = "ai-combinator-code-vars", key_sequence = "CONTROL + F", order = "05" },
    { type = "custom-input", name = "ai-combinator-code-close", key_sequence = "CONTROL + Q", order = "06" },
    { type = "custom-input", name = "ai-combinator-open-gui", key_sequence = "CONTROL + E", order = "07" },
