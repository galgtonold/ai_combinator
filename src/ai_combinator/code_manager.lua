...
function code_manager.load_code(code, uid, source_type)
    ...
    if not (combinator and combinator.e.valid and combinator.out_red.valid and combinator.out_green.valid) then
        return "remove"
    end
    ...
    if not combinator.err_parse then
        for _, player in pairs(game.players) do
            player.remove_alert({ entity = combinator_env._e })
        end
    end
    return nil
end
...
function code_manager.clear_outputs(uid)
    local combinator, combinator_env = storage.combinators[uid], memory.combinators[uid]
    if not (combinator and combinator_env) then
        return
    end
