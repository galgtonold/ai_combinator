...
local function out_wire_connect_both(e)
    return out_wire_connect(e, "red"), out_wire_connect(e, "green")
end
...
function init.combinator_init(e)
    ...
    if memory.combinators[uid] then
        error("Double-init for existing combinator unit_number")
    end
    memory.combinators[uid] = {} -- some state (e.g. loaded func) has to be local
    if not storage.combinators[uid] then
        storage.combinators[uid] = { e = e }
    end
    ...
    local env_wire_red = {
        _e = combinator_env._e,
        _wire = "red",
        _debug = false,
        _out = combinator_env._out,
        _iter = cn.cn_input_signal_iter,
        _cache = {},
        _cache_tick = -1,
    }
    ...
    local env_ro = { -- sandbox.env_base + combinator_env proxies
        ...
        out = setmetatable(combinator_env._out, { __index = cn.cn_output_table_value, __len = cn.cn_output_table_len }),
        red = setmetatable(env_wire_red, {
            __serialize = cn.cn_input_signal_table_serialize,
            __len = cn.cn_input_signal_len,
            __index = cn.cn_input_signal_get,
            __newindex = cn.cn_input_signal_set,
        }),
    ...
    if combinator.core then
        init.out_wire_connect_combinator(combinator)
    end
...
function init.combinator_remove(uid, keep_entities, to_be_mined)
    ...
    if not keep_entities then
        ...
        if not to_be_mined and combinator.e and combinator.e.valid then
            combinator.e.destroy()
        end
    end
