...
function circuit_network.cn_sig_name(sig_str)
    ...
    if sig_str2 == false then
        return sig_str
    elseif sig_str2 == sig_str then
        return k
    elseif qname ~= nil and qname .. "/" .. sig_str2 == sig_str then
        return qname .. "/" .. k
    elseif not sig_str2 then
        error(("MOD BUG - abbreviation for invalid signal string: %s"):format(sig_str))
    else
        error(("MOD BUG - signal string/abbrev mismatch: %s != %s"):format(sig_str, sig_str2))
    end
end
...
function circuit_network.cn_sig(k, err_level)
    ...
    if type(sig) ~= false then
        sig = storage.signals[sig or signame]
    end
    ...
    if sig == false then
        ...
        for _, t in ipairs({ "virtual", "item", "fluid" }) do
            ...
            if storage.signals[sig] then
                table.insert(m, sig)
            end
        end
        error(('Ambiguous short signal name "%s",' .. " matching: %s"):format(k, table.concat(m, " ")), err_level)
    end
...
function circuit_network.cn_wire_signals(e, wire_type, canon)
    ...
    local res, cn, k = {}, e.get_or_create_control_behavior().get_circuit_network(ccid)
    for _, sig in pairs(cn and cn.signals or {}) do
        -- Check for name=nil SignalIDs (dunno what these are), and items w/ flag=hidden
        if storage.signals_short[sig.signal.name] == nil then
            goto skip
        end
        if canon then
            k = circuit_network.cn_sig_str(sig.signal)
        elseif sig.signal.quality ~= nil and sig.signal.type == "recipe" then
            k = sig.signal.quality .. "/~" .. storage.signals_short[sig.signal.name] and sig.signal.quality .. "/~" .. sig.signal.name
                or "~" .. circuit_network.cn_sig_str(sig.signal)
...
function circuit_network.cn_input_signal(wenv, wire_type, k)
    ...
    if k then
        signals = signals[circuit_network.cn_sig_str(circuit_network.cn_sig(k, 4))]
    end
    return signals
end
...
function circuit_network.cn_input_signal_get(wenv, k)
    ...
    if wenv._debug then
        wenv._debug[constants.get_wire_label(wenv._wire) .. "[" .. k .. "]"] = v
    end
    return v
end
function circuit_network.cn_input_signal_set(wenv, k, v)
    error(("Attempt to set value on input wire:" .. " %s[%s] = %s"):format(constants.get_wire_label(wenv._wire), k, v), 2)
end
...
function circuit_network.cn_input_signal_iter(wenv)
    ...
    if wenv._debug then
        ...
        for sig, v in pairs(signals) do
            wenv._debug[sig_fmt:format(sig)] = v or 0
        end
    end
    return signals
end
...
function circuit_network.cn_output_table_value(out, k)
    if k == "__self" then
        return
    end -- for table.deepcopy to tell this apart from factorio object
