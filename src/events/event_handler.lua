        ...
        if known_root_level_events[event_key] then
            ...
            if not root_multiplexers[event_key] then
                ...
                if type(script[event_key]) == "function" then
                    ...
                    log(
                        string.format(
                            "ERROR [EventHandler]: Script key '%s' is in known_root_level_events but script[%s] is not a function!",
                            event_key,
                            event_key
                        )
                    )
    ...
    if type(event_key) == "number" or normal_event then
        ...
        script.on_event(event_key, function(event)
            ...
            if event then
                ...
                if event.element and not event.element.valid then
                    return
                end
                ---@diagnostic disable-next-line: undefined-field
                if event.entity and not event.entity.valid then
                    return
                end
                ---@diagnostic disable-next-line: undefined-field
                if event.player_index and game.players[event.player_index] and not game.players[event.player_index].connected then
                    return
                end
            end
