...
local function on_gui_click(event)
    ...
    if element.tags.edit_test_case_name then
        ...
        set_test_case_name_dialog.show(
            event.player_index,
            element.tags.uid,
            location,
            current_name,
            { uid = element.tags.uid, test_index = element.tags.test_index }
        )
    end
end
...
local function on_test_case_updated(event)
    ...
    if not gui_t or not gui_t.test_case_name_label or not gui_t.test_case_name_label.valid then
        return
    end
