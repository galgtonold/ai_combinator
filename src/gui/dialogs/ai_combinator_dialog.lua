...
local function update_signals(uid)
    ...
    draw_signals(gui_t.input_signal_frame, {}, red_network and red_network.signals or {}, green_network and green_network.signals or {})
    ...
    draw_signals(gui_t.output_signal_frame, signals, {}, {})
end
...
local function update_status(uid)
    ...
    if status_code == defines.entity_status.working or status_code == defines.entity_status.normal then
        status_text = "Working"
    ...
    if combinator.state == "error" then
        status_text = "Error: " .. (combinator.err_parse or combinator.err_run or "unknown error")
        sprite = "utility/status_not_working"
    end
...
function dialog.show(player, entity)
    ...
    local function elc(parent, props, style_tweaks)
        el = parent.add(props)
        for k, v in pairs(style_tweaks or {}) do
            el.style[k] = v
        end
        if props.name then
    ...
    local gui = elc(player.gui.screen, { type = "frame", name = "ai-combinator-gui", direction = "vertical" })
    gui_t.gui = gui
    ...
    elc(
        connections_frame,
        { type = "flow", name = "ai-combinator-connections-flow", direction = "horizontal", style = "player_input_horizontal_flow" }
    )
    ...
    -- Entity preview
    local entity_frame_border =
        elc(entity_frame, { type = "frame", name = "ai-combinator-entity-frame-border", style = "deep_frame_in_shallow_frame" })
    ...
    local progress_flow = elc(
        entity_frame,
        { type = "flow", name = "ai-combinator-progress-flow", direction = "horizontal" },
        { horizontally_stretchable = true, height = 30 }
    )
    local progress_bar = elc(
        progress_flow,
        { type = "progressbar", name = "progressbar", value = 0, style = "production_progressbar", caption = "" },
        { horizontally_stretchable = true }
    )
    progress_bar.style.horizontal_align = "center"
    elc(
        progress_flow,
        { type = "button", name = "ai-combinator-cancel-ai", caption = "Cancel", style = "red_button", tooltip = "Cancel the current AI operation" },
        { width = 80, left_margin = 4, height = 25 }
    )
    ...
    elc(
        entity_frame,
        { type = "button", style = "green_button", name = "ai-combinator-set-task", caption = "Set Task" },
        { horizontally_stretchable = true }
    )

    elc(
        entity_frame,
        { type = "button", style = "button", name = "ai-combinator-edit-code", caption = "Edit Source Code" },
        { horizontally_stretchable = true }
    )
    ...
    local input_output_flow = elc(
        entity_frame,
        { type = "flow", name = "ai-combinator-input-output-flow", direction = "horizontal", style = "inset_frame_container_horizontal_flow" }
    )
