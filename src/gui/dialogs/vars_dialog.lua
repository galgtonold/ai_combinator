...
function dialog.show(pn, uid, paused, toggle_on)
	...
	local gui = player.gui.screen.add{ type='frame',
		name='ai-combinator-vars', caption='', direction='vertical' }
	...
	btns.add{ type='button', name='ai-combinator-vars-pause', caption='Pause',
...
function dialog.update(player, uid, pause_update)
	...
	if gui_paused and pause_update == nil then return end -- ignore calls from combinator updates
	local gui_st_old, gui_st = gui.caption,
		('AI Combinator Environment Variables [%s]%s'):format(uid, gui_paused and ' -- PAUSED' or '')
