        ...
        if #visible_children > 0 then
            if flow_direction == "horizontal" then
                ...
                for _, child in ipairs(visible_children) do
                    ...
                    total_width = total_width + child_size.width + child_spacing.margin_left + child_spacing.margin_right
                    max_height = math.max(max_height, child_size.height + child_spacing.margin_top + child_spacing.margin_bottom)
                end
            ...
            elseif flow_direction == "vertical" then
                ...
                for _, child in ipairs(visible_children) do
                    local child_size = get_actual_size(child)
                    local child_spacing = get_spacing(child)
                    total_height = total_height + child_size.height + child_spacing.margin_top + child_spacing.margin_bottom
                    max_width = math.max(max_width, child_size.width + child_spacing.margin_left + child_spacing.margin_right)
                end
                ...
                for _, child in ipairs(visible_children) do
                    local child_size = get_actual_size(child)
                    local child_spacing = get_spacing(child)
                    total_width = math.max(total_width, child_size.width + child_spacing.margin_left + child_spacing.margin_right)
                    total_height = math.max(total_height, child_size.height + child_spacing.margin_top + child_spacing.margin_bottom)
                end
            end
        end
...
function gui_utils.debug_element_tree(element, indent)
    ...
    local info = string.format("%s%s [%s]: size=%.0fx%.0f", spacing, element.name or "unnamed", element.type, size.width, size.height)
