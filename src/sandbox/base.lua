...
function sandbox.env_pairs(tbl) -- allows to iterate over red/green ro-tables
    ...
    if mt and sandbox.env_pairs_mt_iter[mt.__index] then
        tbl = tbl._iter(tbl)
    end
    return pairs(tbl)
end
...
function sandbox.clean_table(tbl, apply_func)
    local tbl_clean = {}
    for k, v in sandbox.env_pairs(tbl) do
        tbl_clean[k] = v
    end
    if apply_func then
        return apply_func(tbl_clean)
    else
        return tbl_clean
    end
end
...
function sandbox.game_print(...)
    ...
    for _, arg in ipairs(args) do
        ...
        if type(arg) == "table" then
            arg = sandbox.clean_table(arg, serpent.line)
        end
...
sandbox.env_base = {
    ...
    serpent = {
        block = function(tbl)
            return sandbox.clean_table(tbl, serpent.block)
        end,
        line = function(tbl)
            return sandbox.clean_table(tbl, serpent.line)
        end,
    },
    string = {
        byte = string.byte,
...
event_handler.add_handler(defines.events.on_tick, function(event)
    if not sandbox.env_base._init then
        ...
        sandbox.env_base.game = {
            tick = game.tick,
            log = combinator_log,
            print = sandbox.game_print,
            print_color = game.print,
        }
        sandbox.env_base._api = {
            game = game,
            script = script,
            remote = remote,
            commands = commands,
