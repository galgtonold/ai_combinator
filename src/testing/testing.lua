...
function testing.on_test_case_updated(event)
    ...
    if not combinator or not combinator.test_cases or not combinator.test_cases[event.test_index] then
        return
    end
    ...
    local out, actual_print =
        testing.evaluate_test_case(event.uid, signals_to_associative(test_case.red_input), signals_to_associative(test_case.green_input), options)
